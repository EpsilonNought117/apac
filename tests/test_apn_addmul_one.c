...
check_apn_addmul_one(uint64_t iterations)
{
    ...
    while (iterations--)
    {
        ...
        /* TEST-3: addmul_one(..., 0) == no-op */

        apn_set_random(op1, size);
        apn_set_random(op2, size + 1);
        apn_cpy(op3, op2, size + 1);

        carry = apn_addmul_one(op2, op1, size, 0);
        ...
        carry = apn_addmul_one(op2, op1, size, APN_DIG_MAX);
        cmp_res = apn_cmp(op3, op2, size + 1);
...
main(void)
{
    ...
    check_apn_addmul_one(iterations);
